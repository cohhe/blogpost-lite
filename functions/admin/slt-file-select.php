...
if ( ! function_exists( 'add_action' ) ) {
	_e( "Hi there! I'm just a plugin, not much I can do when called directly.", 'vh' );
	exit;
}

// JavaScript
add_action( 'admin_enqueue_scripts', 'slt_fs_scripts' );
function slt_fs_scripts() {
	wp_enqueue_style( 'wp-pointer' );
	wp_enqueue_script( 'wp-pointer' );
	wp_register_script('master', get_template_directory_uri() . '/functions/admin/js/master.js', array('jquery', 'media-upload', 'thickbox', 'jquery-effects-core', 'jquery-effects-bounce'));
	wp_enqueue_script('master');
	$protocol = isset( $_SERVER[ 'HTTPS' ] ) ? 'https://' : 'http://';
	wp_localize_script( 'master', 'slt_file_select', array(
		'ajaxurl'			=> admin_url( 'admin-ajax.php', $protocol ),
		'text_select_file'	=> esc_html__( 'Select', 'vh' ),
		'themeinstall' => admin_url("themes.php?page=themeoptions#first-time-visit")
	));
}

// Styles
add_action( 'admin_enqueue_scripts', 'slt_fs_styles' );
...
function slt_fs_button($name, $value, $label = 'Select file', $preview_size = 'thumbnail', $removable = true, $image = '') {
	...
	<div>
		...
		<?php if ( $value && $removable ) { ?>
			&nbsp;&nbsp;<input type="checkbox" name="<?php echo esc_attr( $name ); ?>_remove" id="<?php echo esc_attr( $name ); ?>_remove" value="1" class="slt-fs-remove" /> <label for="<?php echo esc_attr( $name ); ?>_remove"><?php _e( 'Remove', 'vh' ); ?></label>
		<?php } ?>
